 ...
 */
 ...
 import org.hyperic.sigar.ptql.SimplePTQL.Strategy;
 ...
 public class TestPTQL extends SigarTestCase {
     ...
     private static final String OTHER_JAVA_PROCESS = JAVA_PROCESS + "," + OTHER_PROCESS;
     ...
     private static final String[] OK_QUERIES = {
             ...
             "State.Name.eq=sshd,Cred.Uid.eq=0", "State.Name.eq=crond,Cred.Uid.eq=0",
             "State.State.eq=R", // processes in read state
             "Args.0.eq=sendmail: accepting connections", "Args.0.sw=sendmail: Queue runner@", "Args.1000.eq=foo",
             ...
             "Pid.PidFile.eq=pid.file", "Pid.Pid.eq=1", THIS_PROCESS,
             "Pid.Service.eq=Eventlog", // compat -> Service.Name
             "Service.Name.eq=NOSUCHSERVICE", "Service.Name.ct=Oracle", "Service.DisplayName.re=DHCP|DNS", "Service.Path.ct=svchost", "Service.Exe.Ieq=inetinfo.exe",
             OTHER_JAVA_PROCESS, // all java procs cept this one
             "Cpu.Percent.ge=0.2", "State.Name.sw=java,Args.*.eq=org.jboss.Main", // jboss
     ...
     private static final String[] OK_RE_QUERIES = { "Args.-1.eq=weblogic.Server,Env.WEBLOGIC_CLASSPATH.re=.*weblogic.jar.*", // weblogic
     ...
     private static final String[] MALFORMED_QUERIES = { "foo", "State.Name", "State.Name.eq", "State.Namex.eq=foo", "Statex.Name.eq=foo", "State.Name.eqx=foo",
             "State.Name.Xeq=foo", "State.Name.eq=foo,Args.*.eq=$3", "State.Name.eq=$1", "State.State.eq=read", "Args.x.eq=foo", "Time.Sys.gt=x", "Pid.Pid.eq=foo",
             "Cpu.Percent.ge=x", "Port.foo.eq=8080", "Port.tcp.gt=8080", "Port.tcp.eq=http", "Cpu.Sys.ew=lots", "Service.Invalid.ew=.exe", "", null, };
     ...
     private int runQuery(Sigar sigar, String qs) throws MalformedQueryException, SigarException {
     ...
     public void testValidQueries() throws Exception {
         ...
         String msg = JAVA_PROCESS + " [" + numProcs + "] vs. [" + numOtherProcs + "] " + OTHER_JAVA_PROCESS;
     ...
     public void TestSimplePTQLescape() throws Exception {
         ...
         assertEquals(newVal, "pipe.name=office1");
 
         String part1 = "office.*";
         String whole = part1 + regex;
 
         assertEquals(whole, "office.*\\Qpipe,name=office1\\E");
         ...
         assertEquals(whole.substring(0, index), part1);
         assertEquals(whole.substring(index), regex);
     }
     ...
     public void testSimplePTQLBuilder() throws Exception {
         ...
         assertEquals(ptql.getQuery(), JAVA_PROCESS);
 
         Sigar sigar = getSigar();
         int runQuery = runQuery(sigar, ptql.getQuery());
         assertTrue(runQuery > 0);
     }
 
     public void testSimplePTQLArgs() throws Exception {
         SimplePTQL ptql = new SimplePTQL.Builder(SimplePTQL.STATE_NAME(), SimplePTQL.RE(), "office.*").addArgs("*", SimplePTQL.RE(), "\\Qpipe,name,office1\\E", org.hyperic.sigar.ptql.SimplePTQL.Strategy.ESCAPE)
                 .createQuery();
         assertEquals(ptql.getQuery(), "State.Name.re=office.*,Args.*.re=pipe.name.office1");
 
         ptql = new SimplePTQL.Builder(SimplePTQL.STATE_NAME(), SimplePTQL.EQ(), "office.*").addArgs("*", SimplePTQL.RE(), "\\Qpipe,name,office1\\E", Strategy.ESCAPE).addArgs("*",
                 SimplePTQL.EQ(), "\\Qpipe,name=office2\\E", Strategy.ESCAPE).setStrategy(Strategy.ESCAPE).createQuery();
 
         assertEquals(ptql.getQuery(), "State.Name.eq=office.*,Args.*.re=pipe.name.office1,Args.*.eq=pipe.name=office2");
 
         try {
             ptql = new SimplePTQL.Builder(SimplePTQL.STATE_NAME(), SimplePTQL.EQ(), "office.*").addArgs("1", SimplePTQL.RE(), "\\Qpipe,name,office1\\E", Strategy.ESCAPE).addArgs(
                     "2", SimplePTQL.EQ(), "\\Qpipe,name,office2\\E", Strategy.NOT_ESCAPE).createQuery();
